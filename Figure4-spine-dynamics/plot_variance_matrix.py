    ...
    for batch in np.unique(subdata["batch"]):
        ...
        for culture in np.unique(batch_data["culture"]):
            ...
            for segment in np.unique(culture_data["segment"]):
                ...
                plt.subplot(5,5,i)
                plt.pcolor(ccs,cmap='binary')#,vmin=-0.25,vmax=0.25)
    ...
    for batch in np.unique(subdata["batch"]):
        ...
        for culture in np.unique(batch_data["culture"]):
            ...
            for segment in np.unique(culture_data["segment"]):
                ...
                averaged = average_unit_matrix(ccs)
                plt.subplot(5,5,i)
                plt.pcolor(averaged,cmap='binary')#,vmin=-0.7,vmax=0.7)
                plt.title(str(group)+str(batch)+str(culture)+str(segment))
                plt.colorbar()
...
def plot_two_group_analyze_spine_change_correlation(data,group):
    ...
    img = ax.pcolor(sp.stats.sem(meta,axis=0),cmap='binary',vmin=0.,vmax=0.15)
    ...
    plt.savefig(str(group)+"_matrix_full_sem.svg")
...
def plot_two_increase(data,group):
    ...
    if group == "rMS":
        ...
        cbar.set_label(r"$s.e.m$")
        cax.yaxis.set_label_position('right')
    plt.savefig(str(group)+"_matrix_enlarged_sem.svg")
...
def plot_two_reduced(data,group):
    ...
    if group == "rMS":
        ...
        cbar.set_label(r"$s.e.m$")
        cax.yaxis.set_label_position('right')
    plt.savefig(str(group)+"_matrix_reduced_sem.svg")
