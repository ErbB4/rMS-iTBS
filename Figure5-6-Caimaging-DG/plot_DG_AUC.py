...
#################### get means for plot ##############
    ...
    for idx,culture in enumerate(np.unique(subdata["culture"])):
        ...
        time_1 = culture_data.loc[culture_data["timing"]=="pre"].auc
        time_2 = culture_data.loc[culture_data["timing"]=="3h-post"].auc
        time_3 = culture_data.loc[culture_data["timing"]=="24h-post"].auc
...
#################### get means for two groups ##############
    ...
    means_control = np.array([np.mean(control_data[control_data["timing"]=="3h-post"].auc),np.mean(control_data[control_data["timing"]=="24h-post"].auc)])
    means_rMS = np.array([np.mean(rMS_data[rMS_data["timing"]=="3h-post"].auc),np.mean(rMS_data[rMS_data["timing"]=="24h-post"].auc)])
    sems_control = np.array([sp.stats.sem(control_data[control_data["timing"]=="3h-post"].auc),sp.stats.sem(control_data[control_data["timing"]=="24h-post"].auc)])
    sems_rMS = np.array([sp.stats.sem(rMS_data[rMS_data["timing"]=="3h-post"].auc),sp.stats.sem(rMS_data[rMS_data["timing"]=="24h-post"].auc)])
...
#################### stats ##############
    ...
    rMS_data = batchdata[batchdata["group"]=="rMS"]
    print("3h-stats")
    print(sp.stats.ttest_ind(control_data[control_data["timing"]=="3h-post"].auc,rMS_data[rMS_data["timing"]=="3h-post"].auc))
    print("24h-stats")
    print(sp.stats.ttest_ind(control_data[control_data["timing"]=="24h-post"].auc,rMS_data[rMS_data["timing"]=="24h-post"].auc))
...
def plot_bar(ax,data,region_name):
    ...
    ax.text(5,means[3]+2*sems[3],"ns",fontsize=8,ha='center',va='bottom')
...
md1 = smf.mixedlm("auc ~ group", data_3h, groups=data_3h["culture"])
mdf1 = md1.fit()

md2 = smf.mixedlm("auc ~ group", data_24h, groups=data_24h["culture"])
mdf2 = md2.fit()
