...
#################### get means for plot ##############
    ...
    for idx,culture in enumerate(np.unique(subdata["culture"])):
        ...
        time_1 = culture_data.loc[culture_data["timing"]=="pre"].firing_rate
        time_2 = culture_data.loc[culture_data["timing"]=="3h-post"].firing_rate
        time_3 = culture_data.loc[culture_data["timing"]=="24h-post"].firing_rate
...
#################### get means for two groups ##############
    ...
    means_control = np.array([np.mean(control_data[control_data["timing"]=="3h-post"].firing_rate),np.mean(control_data[control_data["timing"]=="24h-post"].firing_rate)])
    means_rMS = np.array([np.mean(rMS_data[rMS_data["timing"]=="3h-post"].firing_rate),np.mean(rMS_data[rMS_data["timing"]=="24h-post"].firing_rate)])
    sems_control = np.array([sp.stats.sem(control_data[control_data["timing"]=="3h-post"].firing_rate),sp.stats.sem(control_data[control_data["timing"]=="24h-post"].firing_rate)])
    sems_rMS = np.array([sp.stats.sem(rMS_data[rMS_data["timing"]=="3h-post"].firing_rate),sp.stats.sem(rMS_data[rMS_data["timing"]=="24h-post"].firing_rate)])

    return np.array([means_control[0],means_rMS[0],means_control[1],means_rMS[1]])/120.,np.array([sems_control[0],sems_rMS[0],sems_control[1],sems_rMS[1]])/120.
...
#################### stats ##############
    ...
    rMS_data = batchdata[batchdata["group"]=="rMS"]
    print("3h-stats")
    print(sp.stats.ttest_ind(control_data[control_data["timing"]=="3h-post"].firing_rate,rMS_data[rMS_data["timing"]=="3h-post"].firing_rate))
    print("24h-stats")
    print(sp.stats.ttest_ind(control_data[control_data["timing"]=="24h-post"].firing_rate,rMS_data[rMS_data["timing"]=="24h-post"].firing_rate))
...
def autolabel_culture(bars,samplesize,xs):
    for ii,bar in enumerate(bars):
        plt.text(xs[ii], 0.5, '%s'% (samplesize[ii]), fontsize=6,color='w',ha='center', va='bottom')
...
def plot_bar(ax,data,region_name):
    ...
    ax.set_ylabel(r"$\mathrm{Ca^{2+}}$ spikes (Hz)")
    samplesize_culture,samplesize_neuron = plot_individual_datapoint(ax,data,xs,colors)

    ax.text(2,means[1]+2*sems[1],"ns",fontsize=8,ha='center',va='bottom')
...
#################### load data and plot ##############
data = pd.read_csv("calcium_data_DG.csv")
data =data.dropna()

#plot only 3h-post and 24h-post
fig = plt.figure(figsize=(cm2inch(3.7), cm2inch(3.7)))
...
data_update.to_csv("DG_calcium_cleaned.csv")


plot_bar(ax1,data_update,"DG")
stats_pooled(data_update)
ax1.spines['right'].set_visible(False)
ax1.spines['top'].set_visible(False)
ax1.yaxis.set_ticks_position('left')
ax1.xaxis.set_ticks_position('bottom')
plt.savefig("DG-FR.svg")
plt.show()
...
md1 = smf.mixedlm("firing_rate ~ group", data_3h, groups=data_3h["culture"])
mdf1 = md1.fit()

md2 = smf.mixedlm("firing_rate ~ group", data_24h, groups=data_24h["culture"])
mdf2 = md2.fit()

print(mdf1.summary())
print(mdf2.summary(alpha=0.005)) #still significant